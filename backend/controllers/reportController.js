...
exports.getReportSummary = async (req, res) => {
  try {
    ...
    // ---- Appointments (now department-aware)
    const appointmentQuery = { hospitalId: hid };
    if (Object.keys(dateFilter).length) appointmentQuery.slotStart = dateFilter;

    // doctorId filter (single doctor takes priority)
    if (doctorId) {
      appointmentQuery.doctorId = Types.ObjectId.isValid(doctorId) ? new Types.ObjectId(doctorId) : doctorId;
    } else if (department) {
      // department filter -> restrict appointments to doctors in that department
      const deptDocs = await DoctorProfile.find({
        hospitalId: hid,
        specialties: { $in: [department] }
      }).select('userId');
      const doctorIds = deptDocs.map(d => d.userId);
      // If no doctors in that department, short-circuit to empty results
      appointmentQuery.doctorId = doctorIds.length ? { $in: doctorIds } : { $in: [] };
    }
    ...
    // ---- Payments (cannot be department-filtered unless payments reference doctor/department)
    ...
    const [doctorCount, topDoctors, doctorsBySpecialty] = await Promise.all([
      ...
      Appointment.aggregate([
        { $match: appointmentQuery }, // already filtered by dept if provided
    ...
    // ---- Patient Reports (not department-scoped)
    ...
    // ---- Comparison (inherits appointmentQuery filters incl. department)
    ...
    // ---- Top Medications (payments aren’t dept scoped)
...
/* ==================== 6) DOWNLOAD REPORT ==================== */
// (kept your latest version unchanged to avoid mixing concerns)
