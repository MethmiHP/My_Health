...
// ✅ use the same helpers you use elsewhere in the app
import { API_BASE, useAuthHeaders, postJSON } from "../../utils/api";
import hospitalBg from "../../assets/steth.jpg";
const API = import.meta.env.VITE_API_BASE_URL || API_BASE || "http://localhost:5000";
...
const CardButton = ({ icon: Icon, title, desc, onClick, variant = "default" }) => (
  ...
  >
    <div className="flex items-center gap-3">
      <div className={`rounded-xl ${variant === "reports" ? "bg-purple-100" : "bg-teal-50"} p-3`}>
        <Icon className={`h-6 w-6 ${variant === "reports" ? "text-purple-700" : "text-teal-700"}`} />
      </div>
      <div>
        <h3 className={`text-lg font-semibold ${variant === "reports" ? "text-purple-900" : "text-teal-900"}`}>{title}</h3>
        <p className={`${variant === "reports" ? "text-purple-900/70" : "text-teal-900/70"} text-sm`}>{desc}</p>
      </div>
    </div>
  </button>
);
...
const Field = ({ label, icon: Icon, className = "", ...props }) => (
  <label className={`block ${className}`}>
    ...
    <div className="relative">
      ...
      <input
        {...props}
        className={`w-full rounded-xl border border-teal-200 bg-white ${Icon ? "pl-10" : "pl-4"} pr-4 py-3 text-sm outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400`}
      />
    </div>
  </label>
);
...
const Select = ({ label, icon: Icon, className = "", children, ...props }) => (
  <label className={`block ${className}`}>
    ...
    <div className="relative">
      ...
      <select
        {...props}
        className={`w-full appearance-none rounded-xl border border-teal-200 bg-white ${Icon ? "pl-10" : "pl-4"} pr-10 py-3 text-sm outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400`}
      >
        {children}
      </select>
...
const CreateDoctorModal = ({ open, onClose, onCreated }) => {
  ...
  const role = "doctor"; // explicit role field (read-only for this modal)
  ...
  const updateSlot = (idx, field, value) => setAvailability((p) => p.map((s, i) => (i === idx ? { ...s, [field]: value } : s)));
      ...
      <div className="min-h-full flex items-start justify-center p-4">
        <div className="w-full max-w-3xl rounded-2xl bg-white border border-teal-100 shadow-xl">
          ...
          <form onSubmit={submit} className="p-6 space-y-6">
            {/* Role (read-only for clarity) */}
            <Select label="Role" disabled value={role}>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-700 focus:ring-2 focus:ring-teal-400 ${loading ? "opacity-60 cursor-not-allowed" : ""}`}
              >
...
const CreatePatientModal = ({ open, onClose, onCreated }) => {
  ...
  const role = "patient"; // explicit role (read-only)
  ...
  const updateSurgery = (idx, field, value) => setSurgeries((p) => p.map((s, i) => (i === idx ? { ...s, [field]: value } : s)));

  const isUnder16 = () => {
    if (!dob) return false;
    const age = Math.floor((new Date() - new Date(dob)) / (365.25 * 24 * 60 * 60 * 1000));
    return age <= 16;
    // NOTE: your NIC checks below depend on this.
  };
      ...
      <div className="min-h-full flex items-start justify-center p-4">
        <div className="w-full max-w-3xl rounded-2xl bg-white border border-teal-100 shadow-xl">
          <div className="p-6 border-b border-teal-100">
            ...
            <p className="text-sm text-teal-900/70">Register a patient. A barcode will be generated automatically.</p>
          </div>
...
const CreateCashierModal = ({ open, onClose, onCreated }) => {
  ...
  const role = "cashier"; // explicit role (read-only)
...
export default function HospitalDashboard() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Header */}
      <section className="bg-white/95 backdrop-blur-sm border-b border-teal-100">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-teal-900 text-shadow-sm">Hospital Dashboard</h1>
              <p className="text-sm text-teal-900/70">Manage staff, patients, and view comprehensive analytics</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-2xl border border-teal-100 bg-white p-6">
            ...
            <ul className="text-sm text-teal-900/80 list-disc list-inside space-y-1">
              ...
              <li>
                ...
                <code className="bg-teal-50 px-1 rounded">PT-&lt;HOSPITAL_CODE&gt;-&lt;user_id&gt;</code>
              </li>
      ...
      {/* Modals */}
      <CreateDoctorModal open={open === "doctor"} onClose={() => setOpen(null)} onCreated={() => {}} />
      <CreatePatientModal open={open === "patient"} onClose={() => setOpen(null)} onCreated={() => {}} />
      <CreateCashierModal open={open === "cashier"} onClose={() => setOpen(null)} onCreated={() => {}} />
      </main>
    </div>
  );
}
